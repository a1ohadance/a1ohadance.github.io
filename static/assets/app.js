...
(function(){
    ...
    function init(){
      ...
      try{
        var orgs=[
          ...
          {name:"US DoE",loc:"Washington, D.C. · United States",lat:38.90,lng:-77.03,host:"energy.gov",logo:"hof_doe_c.png",cal:{a:40,l:90}},
          {name:"Nokia",loc:"Espoo · Finland",lat:60.21,lng:24.66,host:"nokia.com",logo:"hof_nokia_c.png",cal:{a:-90,l:44}},
          {name:"BP",loc:"London · United Kingdom",lat:51.51,lng:-0.13,host:"bp.com",logo:"hof_bp_c.png",cal:{a:170,l:60}},
          {name:"Motorola",loc:"Chicago · United States",lat:41.88,lng:-87.63,host:"motorolasolutions.com",logo:"hof_moto_c.png",cal:{a:-170,l:66}}
        ];
        var GPAL=[["#00e5ff","0,229,255"],["#ff3344","255,51,68"],["#ff2ec4","255,46,196"],["#ffae00","255,174,0"],["#c6ff3a","198,255,58"],["#9b5cff","155,92,255"],["#2be06a","43,224,106"],["#ff7a1a","255,122,26"],["#3b82f6","59,130,246"],["#00d47f","0,212,127"],["#ff5fa2","255,95,162"]];
        /* NASA=cyan WHO=red UNICEF=magenta UNESCO=amber Ferrari=lime Bayer=violet US-DoEd=green US-DoE=orange Nokia=blue BP=spring-green Motorola=rose (all distinct) */
        ...
        if(recFan){ recFan.innerHTML=""; var mid=(orgs.length-1)/2, fgap=orgs.length>9?26:30, farc=orgs.length>9?4:5;
          orgs.forEach(function(o,ci){ var cx=Math.round((ci-mid)*fgap),cy=Math.round(Math.abs(ci-mid)*farc),cr=((ci-mid)*5).toFixed(1);
        ...
        function step(){
          ...
          if(cur===0) log(cs("▰▰▰ NEW SWEEP · "+orgs.length+" TARGETS QUEUED ▰▰▰","thdrnew"),"");
  ...
  /* REBOOT — Kali XFCE logout dialog -> systemd restart log -> reload home so the
     8-bit boot loader replays (respects prefers-reduced-motion: skips straight to reload). */
  function wireBootReplay(){
    var btn=document.getElementById("bootReplay"); if(!btn) return;
    var reduce = window.matchMedia && matchMedia("(prefers-reduced-motion:reduce)").matches;
    var STOP=[
      '<span class="kr-dim">Broadcast message from root@kali (pts/0):</span>',
      '<span class="kr-warn">The system is going down for reboot NOW!</span>','&nbsp;',
      '[<span class="kr-ok">  OK  </span>] Stopped target <b>Graphical Interface</b>.',
      '[<span class="kr-ok">  OK  </span>] Stopped <b>Signal Jam</b> service.',
      '[<span class="kr-ok">  OK  </span>] Stopped <b>OpenVPN</b> connection.',
      '[<span class="kr-ok">  OK  </span>] Unmounted <b>/dev/wallpaper</b>.',
      '[<span class="kr-ok">  OK  </span>] Reached target <b>Shutdown</b>.',
      '[<span class="kr-ok">  OK  </span>] Reached target <b>Reboot</b>.',
      '<span class="kr-dim">Rebooting.</span>'
    ];
    var START=[
      '<span class="kr-dim">Booting a1ohadance GNU/Linux ...</span>','&nbsp;',
      '[<span class="kr-ok">  OK  </span>] Started <b>Journal Service</b>.',
      '[<span class="kr-ok">  OK  </span>] Mounted <b>/dev/wallpaper</b>.',
      '[<span class="kr-ok">  OK  </span>] Reached target <b>Basic System</b>.',
      '[<span class="kr-ok">  OK  </span>] Started <b>Signal Jam</b> service.',
      '[<span class="kr-ok">  OK  </span>] Started <b>Recon Globe</b> daemon.',
      '[<span class="kr-ok">  OK  </span>] Reached target <b>Graphical Interface</b>.'
    ];
    function boot(){ try{ sessionStorage.removeItem("a1oha-booted"); }catch(e){} window.location.href="/"; }
    function typeLines(el,lines,delay,done){ var i=0;(function n(){ if(i>=lines.length){done&&done();return;}
      var r=document.createElement("div"); r.innerHTML=lines[i]; el.appendChild(r); el.scrollTop=el.scrollHeight; i++; setTimeout(n,delay); })(); }
    function removeOv(){ var o=document.getElementById("krOv"); if(o&&o.parentNode) o.parentNode.removeChild(o); }
    function runSeq(o){
      o.innerHTML='<div class="kr-term" id="krT"></div>';
      var t=o.querySelector("#krT");
      typeLines(t,STOP,140,function(){ setTimeout(function(){ t.innerHTML="";
        typeLines(t,START,140,function(){ setTimeout(function(){
          var f=document.createElement("div"); f.className="kr-flash"; o.appendChild(f);
          requestAnimationFrame(function(){ f.classList.add("kr-go"); }); setTimeout(boot,440);
        },350); }); },500); });
    }
    function openDialog(){
      var restart='<svg viewBox="0 0 24 24"><path d="M4 12a8 8 0 1 0 2.3-5.6"/><path d="M4 4v4h4"/></svg>';
      var o=document.createElement("div"); o.className="kr-ov"; o.id="krOv";
      o.innerHTML='<div class="kr-dlg"><div class="kr-confirm"><span class="kr-cicon">'+restart+'</span>'+
        '<div class="kr-ctitle">REBOOT?</div>'+
        '<div class="kr-crow"><button class="kr-no" data-a="x">NO</button><button class="kr-yes" data-a="go">YES</button></div></div></div>';
      document.body.appendChild(o);
      o.querySelectorAll(".kr-crow button").forEach(function(b){ b.addEventListener("click",function(){ if(b.getAttribute("data-a")==="go"){ runSeq(o); } else { removeOv(); } }); });
      o.querySelector(".kr-dlg").addEventListener("click",function(e){ if(e.target===o.querySelector(".kr-dlg")) removeOv(); });
    }
    btn.addEventListener("click", function(){ if(reduce){ boot(); return; } openDialog(); });
    document.addEventListener("keydown", function(e){ if(e.key==="Escape") removeOv(); });
  }

  function wireHofReveal(){
    var chips=document.querySelectorAll(".hof-chip");
    if(!chips.length) return;
    if(window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    var DPR=Math.min(2,window.devicePixelRatio||1);
    function play(chip){
      if(chip._fxRun) return;
      var fx=chip.querySelector(".hof-fx"),base=chip.querySelector(".hof-base"),canvas=fx&&fx.querySelector("canvas");
      if(!fx||!base||!canvas) return;
      var FS=+chip.getAttribute("data-form")||4900,FC=+chip.getAttribute("data-crisp")||FS+1400,HOLD=520;
      var isSvg=/\.svg(\?|$)/i.test(base.getAttribute("src")||"");
      chip._fxRun=true;fx.classList.remove("fx-fade");chip.classList.add("fx-on");
      var ctx=canvas.getContext("2d"),W,H,P=[],S=Math.max(2,Math.round(2*DPR)),ready=false,t0=performance.now(),stop=false;
      function sample(){
        W=canvas.width=Math.round(fx.clientWidth*DPR);H=canvas.height=Math.round(fx.clientHeight*DPR);
        if(!W||!H)return;
        var pad=6*DPR,dx,dy,dw,dh,off=document.createElement("canvas");off.width=W;off.height=H;var o=off.getContext("2d");
        if(isSvg){var ar=parseFloat(base.getAttribute("data-ar"))||1,s2=Math.min((W-2*pad)/ar,(H-2*pad));dw=ar*s2;dh=s2;dx=(W-dw)/2;dy=(H-dh)/2;}
        else{var iw=base.naturalWidth||100,ih=base.naturalHeight||100,s=Math.min((W-2*pad)/iw,(H-2*pad)/ih);dw=iw*s;dh=ih*s;dx=(W-dw)/2;dy=(H-dh)/2;}
        try{o.drawImage(base,dx,dy,dw,dh);}catch(e){return;}
        var d;try{d=o.getImageData(0,0,W,H).data;}catch(e){return;}
        var gap=Math.max(3,Math.round(3*DPR));P=[];
        for(var y=0;y<H;y+=gap)for(var x=0;x<W;x+=gap){var i=(y*W+x)*4,a=d[i+3];if(a<110)continue;var r=d[i],g=d[i+1],b=d[i+2];if(r>245&&g>245&&b>245)continue;P.push({tx:x,ty:y,x:0,y:0,col:"rgb("+r+","+g+","+b+")"});}
        for(var k=0;k<P.length;k++){var p=P[k],an=Math.random()*6.28,rr=(0.4+Math.random()*0.5)*W;p.x=W/2+Math.cos(an)*rr;p.y=H/2+Math.sin(an)*rr*0.6;}
        ready=P.length>0;
      }
      function finish(){fx.classList.add("fx-fade");setTimeout(function(){chip.classList.remove("fx-on");fx.classList.remove("fx-fade");if(ctx&&W)ctx.clearRect(0,0,W,H);chip._fxRun=false;},360);}
      function frame(now){
        if(stop)return;
        if(!ready){requestAnimationFrame(frame);return;}
        var c=now-t0;
        if(c<FS){ctx.clearRect(0,0,W,H);requestAnimationFrame(frame);return;}
        ctx.clearRect(0,0,W,H);ctx.shadowBlur=3*DPR;
        if(c<FC){for(var i=0;i<P.length;i++){var p=P[i];p.x+=(p.tx-p.x)*0.12;p.y+=(p.ty-p.y)*0.12;ctx.shadowColor=p.col;ctx.fillStyle=p.col;ctx.fillRect(p.x,p.y,S,S);}requestAnimationFrame(frame);return;}
        for(var j=0;j<P.length;j++){var q=P[j];ctx.shadowColor=q.col;ctx.fillStyle=q.col;ctx.fillRect(q.tx,q.ty,S,S);}
        stop=true;setTimeout(finish,HOLD);
      }
      if(base.complete && base.naturalWidth){sample();}else{base.addEventListener("load",sample,{once:true});}
      requestAnimationFrame(frame);
    }
    if("IntersectionObserver" in window){
      var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){play(e.target);io.unobserve(e.target);}});},{threshold:0.4});
      chips.forEach(function(ch){io.observe(ch);});
    }else{chips.forEach(play);}
    chips.forEach(function(ch){ch.addEventListener("mouseenter",function(){play(ch);});});
  }

  // colorful per-letter scramble page titles (.dl-pagetitle on education/experience/recognition)
  function wireColorTitle(){
    if(window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    var G="!<>-_\\/[]{}=+*^?#01";
    document.querySelectorAll(".dl-pagetitle").forEach(function(pt){
      var word=(pt.textContent||"").trim();if(!word||pt._ct)return;pt._ct=true;
      pt.innerHTML=word.split("").map(function(c){if(c===" ")return '<span class="l" data-c=" ">&nbsp;</span>';return '<span class="l" data-c="'+c+'">'+c+'</span>';}).join("");
      var ls=pt.querySelectorAll(".l");
      function scr(){var f=0,total=28;var iv=setInterval(function(){f++;ls.forEach(function(sp,i){var d=sp.getAttribute("data-c");if(d===" ")return;var th=5+i*3;sp.textContent=f>=th?d:G[Math.floor(Math.random()*G.length)];});if(f>=total){clearInterval(iv);ls.forEach(function(sp){sp.textContent=sp.getAttribute("data-c");});setTimeout(scr,3400);}},45);}
      setTimeout(scr,900);
    });
  }
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    wirePageFx();
    wirePong();
    wireHofReveal(); // hall-of-fame logo reveal (scroll-in + hover)
    wireColorTitle(); // green scramble page titles (edu/exp/recognition)

    // wireDecode(); // content scramble — DISABLED for now (per request; re-enable to bring it back)
...
(function(){
  ...
  if(!b) return;
  var nraf=null,gi=null;
  function rm(id){ var el=document.getElementById(id); if(el&&el.parentNode) el.parentNode.removeChild(el); }
  function kill(){ if(nraf){cancelAnimationFrame(nraf);nraf=null;} if(gi){clearInterval(gi);gi=null;} if(b&&b.parentNode){ b.parentNode.removeChild(b); b=null; } rm("boot-desat"); rm("boot-flash"); }
  b.addEventListener("click",kill);
  document.addEventListener("keydown",function(e){ if(e.key==="Escape"||e.key===" "||e.key==="Enter") kill(); });
  /* REAL signal jam: green static re-randomised every frame on the boot canvas */
  if(document.documentElement.classList.contains("booting")){
    var cv=document.getElementById("boot-noise");
    if(cv&&cv.getContext){ var ctx=cv.getContext("2d");
      setTimeout(function(){ if(!b||!ctx) return;
        var vw=window.innerWidth||1000, vh=window.innerHeight||620;
        cv.width=Math.min(vw,960); cv.height=Math.max(1,Math.round(cv.width*vh/vw));   // fine grain, ~HD
        var img=ctx.createImageData(cv.width,cv.height),d=img.data,last=0,t0n=0;
        for(var k=3;k<d.length;k+=4){ d[k]=255; }
        function loop(t){ if(!b||!cv.isConnected) return; if(!t0n)t0n=t; if(t-last>33){
            var mono=(t-t0n)>1500;   // colored RGB glitch first, then DRAIN to mono (grayscale) before the B/W strobe
            for(var i=0;i<d.length;i+=4){ if(mono){ var v=(Math.random()*255)|0; d[i]=d[i+1]=d[i+2]=v; } else { d[i]=(Math.random()*255)|0; d[i+1]=(Math.random()*255)|0; d[i+2]=(Math.random()*255)|0; } }
            ctx.putImageData(img,0,0); last=t; }
          nraf=requestAnimationFrame(loop); }
        nraf=requestAnimationFrame(loop); }, 1500);
    }
    /* row scramble — fires AFTER the static (2.0s), corrupting the boot log, then restores */
    setTimeout(function(){ if(!b) return;
      var labs=[].slice.call(b.querySelectorAll(".boot-log .lbl"));
      labs.forEach(function(l){ if(l._o==null) l._o=l.textContent; });
      var CH="!<>-_\\/[]=+*?#01ABXZ%&$@", t0=performance.now();
      gi=setInterval(function(){ if(!b){ clearInterval(gi); gi=null; return; } var p=(performance.now()-t0)/800;
        labs.forEach(function(l){ var fin=l._o,o=""; for(var i=0;i<fin.length;i++){ o+=(fin[i]===" ")?" ":CH[(Math.random()*CH.length)|0]; } l.textContent=o; });
        if(p>=1){ clearInterval(gi); gi=null; labs.forEach(function(l){ l.textContent=l._o; }); }
      },55);
    }, 2000);
  }
  setTimeout(kill,5900);
})();
...
/* ===== DMG-LCD education: CGPA count-up ===== */
(function(){
  var els = document.querySelectorAll(".dl-cgc");
  if(!els.length) return;
  var reduce = window.matchMedia && matchMedia("(prefers-reduced-motion:reduce)").matches;
  function run(el){
    ...
    function fin(){ done=true; el.textContent = to.toFixed(2); }
    function step(ts){ if(done) return; if(!start) start=ts; var k=Math.min(1,(ts-start)/dur), e=k*k*(3-2*k); el.textContent=(to*e).toFixed(2); if(k<1) requestAnimationFrame(step); else fin(); }
    requestAnimationFrame(step);
    setTimeout(fin, dur+250);
  }
  [].forEach.call(els, run);
})();
